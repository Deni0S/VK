...
final class FriendViewController: UITableViewController, UISearchBarDelegate {
    ...
    // Загрузить данные
    func loadFriendData() {
        VKServiceProxy().getFriend() { [weak self] error in
            if let error = error {
                print(error)
                return
            }
    ...
    override func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        let view = UIView()
        view.backgroundColor = .rgbaCache(128.0, 128.0, 128.0, 1.0)
        view.alpha = 0.5
