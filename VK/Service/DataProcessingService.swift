...
//  VK
//
//  Created by Денис Баринов on 21.4.20.
...
class DataProcessingService {
    ...
    // MARK: - Преобразовать дату в установленый формат
    ...
    private static let pathName: String = {
        let pathName = "images"
        guard
            let cachesDirectory = FileManager.default.urls(for: .cachesDirectory, in: .userDomainMask).first else { return pathName }
    ...
    private func getImageFromCache(url: String) -> UIImage? {
        ...
        guard lifeTime <= casheLifeTime, let image = UIImage(contentsOfFile: fileName) else { return nil }
        images[url] = image
        return image
    }
...
extension DataProcessingService {
