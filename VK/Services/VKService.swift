...
final class VKService: VKServiceInterface {
    ...
    private let photoQueue = DispatchQueue(label: "Photo Queue", qos: .utility)
    private let friendQueue = DispatchQueue(label: "Friend Queue", qos: .utility)
    private let groupQueue = DispatchQueue(label: "Group Queue", qos: .utility)
